...
def run (eq,timeout,ploc,wd,solver="1",param="60"):
    path = ploc.findProgram ("nielsen-transformation")
    if not path:
        raise "nielsen-transformation not in path"
    tempd = tempfile.mkdtemp ()
    smtfile = os.path.join (tempd,"nielsen_transformation_out.smt")
    ...
    time = timer.Timer ()
    try:
        out = subprocess.check_output ([path,smtfile],timeout=int(timeout)).decode().strip()
...
def addRunner (addto):
    addto['nielsen-transformation'] = run
